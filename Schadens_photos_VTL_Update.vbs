...
fname = "E:\Schnittstellen\_Scripte\FLAG\" & "Schadens_fotos_VTL_UPDATE.flag"
if (TestWorkingFlag(fname)) then 
WriteTEXTLog " Flag"
else 
WriteTEXTLog " Begin"
CreateWorkingFlag(fname)
...
End With
   ...
   Set result = con.Execute(sql)
	...
	WScript.Sleep 1000
        Runbefehl "magick.exe " & PatchJpg & YMD  & ArcDocINr & ".jpg" & " -strip -quality 75 " & PatchJpg & YMD  & ArcDocINr & ".jpg"
        WScript.Sleep 3000
	WriteTEXTLog "magick.exe " & PatchJpg & YMD  & ArcDocINr & ".jpg" & " -strip -quality 75 " & PatchJpg & YMD  & ArcDocINr & ".jpg"
	    ...
	    writelog ITOg, pathcXML&FileNameOUT
            ...
            WriteTEXTLog PatchJpg & FileNameIN
            'BackUpFile PatchJpg , pathcarj
...
DeleteWorkingFlag(fname)
WriteTEXTLog " End" 
end if

function CreateWorkingFlag(fname)
	Dim fso, ts
    Const ForAppending = 8
	Set fso = CreateObject("Scripting.FileSystemObject")
	Set objLogFile = FSO.OpenTextFile(fname , ForAppending, TRUE)
    objLogFile.WriteLine(fname)
    objLogFile.close 
    
end function

function TestWorkingFlag(fname)
	Dim fso, ts
	Set fso = CreateObject("Scripting.FileSystemObject")
	if fso.FileExists(fname) then
	  TestWorkingFlag = true
	else
	 TestWorkingFlag= false
	end if
end function

function DeleteWorkingFlag(fname)
	Dim fso0, ts
	Set fso0 = CreateObject("Scripting.FileSystemObject")
	fso0.DeleteFile(fname)
end function
...
For Each File in Collec_Files
    PEREMENN = 0
     ...
     Else
       ...
       'WScript.Sleep 100
       Second_ss = (second(now))
       Datatime_YY = Year_yy & Month_MM & Day_dd & Hour_HH & Minute_mm & PEREMENN & "_" & Second_ss  
